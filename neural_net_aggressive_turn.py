...
names_data = pd.read_csv('aggressive_turn.csv')
...
features = ['card1','card2','card3','card4','card5','card6','bet1','bet2','bet3']
...
#
# torch.save(model.state_dict(),'checkpoint_agg_turn.pth')

state_dict_agg_turn = torch.load('checkpoint_agg_turn.pth')

model.load_state_dict(state_dict_agg_turn)

def test_cards_agg_turn(card):
    with torch.no_grad():
