...
names_data = pd.read_csv('tight_flop.csv')
...
#
# torch.save(model.state_dict(),'checkpoint_tight_flop.pth')

state_dict_tig_flop = torch.load('checkpoint_tight_flop.pth')

model.load_state_dict(state_dict_tig_flop)

def test_cards_tig_flop(card):
    with torch.no_grad():
