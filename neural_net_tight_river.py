...
names_data = pd.read_csv('tight_river.csv')
...
#
# torch.save(model.state_dict(),'checkpoint_tight_river.pth')

state_dict_tig_river = torch.load('checkpoint_tight_river.pth')

model.load_state_dict(state_dict_tig_river)

def test_cards_tig_river(card):
    with torch.no_grad():
